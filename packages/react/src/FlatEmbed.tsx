...
import type { FlatEmbedProps, FlatEmbedHandle, FlatEmbedConfig } from './types';
 ...
 * ```
 */
export const FlatEmbed = forwardRef<FlatEmbedHandle, FlatEmbedProps>((props, ref) => {
  const {
    config: configProp,
    score: scoreProp,
    appId: appIdProp,
  ...
  const config: FlatEmbedConfig = {
    ...
    embedParams: {
      ...
      ...(context?.appId && !appIdProp && !configProp?.embedParams?.appId && { appId: context.appId }),
    },
  };
  ...
  useEffect(() => {
    ...
    if (onPlaybackPosition) embed.on('playbackPosition', onPlaybackPosition as any);
    ...
    return () => {
      ...
      if (onCursorPosition) embed.off('cursorPosition', onCursorPosition as any);
      ...
      if (onPlaybackPosition) embed.off('playbackPosition', onPlaybackPosition as any);
  ...
  useImperativeHandle(ref, () => embedRef.current as Embed, [embedRef.current]);
