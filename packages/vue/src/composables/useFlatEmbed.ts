...
export function useFlatEmbed(
  ...
  onMounted(() => {
    ...
    if (onCursorContext) embedInstance.on('cursorContext', onCursorContext as any);
    if (onMeasureDetails) embedInstance.on('measureDetails', onMeasureDetails as any);
    if (onNoteDetails) embedInstance.on('noteDetails', onNoteDetails as any);
    if (onRangeSelection) embedInstance.on('rangeSelection', onRangeSelection as any);
    if (onFullscreen) embedInstance.on('fullscreen', onFullscreen as any);
    if (onRestrictedFeatureAttempt) embedInstance.on('restrictedFeatureAttempt', onRestrictedFeatureAttempt as any);
  });
