...
class Embed {
	...
	constructor(
		...
		if (
			normalizedElement instanceof HTMLIFrameElement &&
			embeds.has(normalizedElement)
		) {
	 ...
	 */
	loadFlatScore(
		score: string | { score: string; sharingKey?: string },
	): Promise<void> {
	...
	getCursorPosition(): Promise<NoteCursorPosition> {
		return this.call(
			"getCursorPosition",
		) as unknown as Promise<NoteCursorPosition>;
	}
