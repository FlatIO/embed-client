			...
			if (trimmed.startsWith("* @example") || trimmed.startsWith("*@example")) {
				...
				if (
					...
					if (cleaned.trim().match(/^-\s+/)) {
						...
						const enumMatch = listItem.match(
							/^-\s+(\d+):\s+([A-Z_]+)\s*-?\s*(.*)$/,
						);
						if (enumMatch) {
...
`;
				...
				const exampleParams = method.parameters.map((p) => {
					...
					if (p.type.includes("{") && p.type.includes("}")) {
						...
						if (objMatch) {
							...
							props.forEach((prop) => {
								const [propNameRaw, propType] = prop
									.split(":")
									.map((s) => s.trim());
			...
			if (method.returnType.includes("Promise")) {
				if (method.name.startsWith("get") || method.name === "ready") {
					const resultVar =
						method.name.replace(/^get/, "").charAt(0).toLowerCase() +
						method.name.slice(4);
	...
	if (method.see && method.see.length > 0) {
		...
		method.see.forEach((s) => {
			...
			if (linkMatch) {
				...
				// Check if it's a URL
				if (
					linkedTarget.startsWith("http://") ||
					linkedTarget.startsWith("https://")
				) {
					// Create clean text for common Flat API URLs
					let linkText = linkedTarget;
					if (
						linkedTarget.includes(
							"flat.io/developers/api/reference/#operation/",
						)
					) {
...
async function main() {
	...
	if (!existsSync(typeDefsPath)) {
		console.error(
			"❌ TypeScript definitions not found. Run `npm run build` first.",
		);
		process.exit(1);
	}
	...
	if (existsSync(typesDir)) {
		const typeFiles = readdirSync(typesDir).filter((file) =>
			file.endsWith(".d.ts"),
		);
