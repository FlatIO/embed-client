const APP_ID =
	window.__karma__.config.env.FLAT_EMBED_APP_ID || "58fa312bea9bbd061b0ea8f3";
const BASE_URL =
	window.__karma__.config.env.FLAT_EMBED_BASE_URL || "https://flat-embed.com";
const PUBLIC_SCORE =
	window.__karma__.config.env.FLAT_EMBED_PUBLIC_SCORE ||
	"56ae21579a127715a02901a6";
const QUARTET_SCORE =
	window.__karma__.config.env.FLAT_EMBED_QUARTET_SCORE ||
	"5e1348dd6d09386a2b178b58";
const PRIVATE_LINK_SCORE =
	window.__karma__.config.env.FLAT_EMBED_PRIVATE_LINK_SCORE ||
	"5ce56f7c019fd41f5b17b72d";
...
describe("Integration - Embed", () => {
	...
	describe("Loading embed", () => {
		...
		it("should plug into an existing iframe", (done) => {
			...
			iframe.setAttribute(
				"src",
				`${baseUrl}/${PUBLIC_SCORE}?jsapi=true&appId=${APP_ID}`,
			);
	...
	describe("MusicXML import/export", () => {
		it("shoud load a MusicXML (plain) in a blank embed", (done) => {
			...
			fetch(
				"/base/test/integration/fixtures/flat-house-of-the-rising-sun.musicxml",
			)
				.then((response) => {
		...
		it("shoud export a score into a plain MusicXML", (done) => {
			...
			fetch("/base/test/integration/fixtures/flat-house-of-the-rising-sun.mxl")
				...
				.then((xml) => {
					assert.ok(
						xml.includes("<work-title>House of the Rising Sun</work-title>"),
					);
					container.parentNode.removeChild(container);
					done();
				})
	...
	describe("Load Flat score", () => {
		...
		it("should load a privateLink score using loadFlatScore()", async () => {
			...
			assert.equal(
				meta.title,
				"House of the Rising Sun (Private link test Embed)",
			);
		...
		it("should set fail to set cursor with bad param value", (done) => {
			...
			embed
				...
				.catch((error) => {
					assert.equal(
						error.message,
						"Parameter measureIdx should be a number, not boolean",
					);
					done();
				});
		});
